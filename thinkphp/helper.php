...
if (!function_exists('dump')) {
    ...
    {
        Debug::dump($var, $echo, $label);exit();
    }
}
...
if ( !function_exists('wlog')) {
     ...
     * @param int FILE_APPEND
     * @param string $type 数据输出的类型默认序列化
     * @param string $pathName 输出的目录
     * @param boolean $isShowTime 是否要插入生成时间
     * @auth 项羽大大
     */
    function wlog($info, $fileName, $flags = FILE_APPEND, $type = 'serialize', $pathName = 'Log', $isShowTime = true)
    {
        //组合路径
        $path = $_SERVER['DOCUMENT_ROOT'] . '/' . trim($pathName, '/') . '/' . date('Ymd');
        //判断目录是否存在
        ...
        $runTime = number_format(microtime(true) - RUNTIME, 6);
        //打印文本
        if ($isShowTime) {
            file_put_contents($path . '/' . $fileName, '日志id' . LOGGERID . '--->' . '[' . date(DATE_ISO8601) . '][info]' . $info . '--->运行时间' . $runTime . "\n", $flags);
        } else {
            file_put_contents($path . '/' . $fileName, '日志id' . LOGGERID . '--->' . $info . '--->运行时间' . $runTime, $flags);
        }
    }
}
